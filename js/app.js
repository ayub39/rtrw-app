...
function badge(status) {
  const map = { 'Baru': 'blue', 'Menunggu': 'orange', 'Belum': 'red', 'Diproses': 'orange', 'Dibuka': 'green', 'Selesai': 'green', 'Lunas': 'green', 'Ditolak': 'red', 'Disetujui': 'green', 'Ditutup': 'gray' };
...
function emptyState(msg, icon) { return `<div class="empty"><span class="empty-ic">${ic(icon || 'clipboard')}</span><p>${esc(msg)}</p></div>`; }
function skeleton(n) {
  n = n || 3; let s = '<div class="skel-wrap">';
...
function confirmModal(msg, opts) {
  ...
  return new Promise((resolve) => {
    ...
    wrap.innerHTML = `<div class="modal"><div class="modal-body">${esc(msg)}</div>
      <div class="modal-actions"><button class="btn ghost" data-mno>${esc(opts.cancelText || 'Batal')}</button>
      <button class="btn ${opts.danger ? 'danger' : 'primary'}" data-myes>${esc(opts.okText || 'Ya')}</button></div></div>`;
...
function injectSuratStyles() {
  ...
  const st = document.createElement('style'); st.id = 'surat-modal-css';
  st.textContent = `
...
function listToolbar(statuses) {
  ...
  return `<div class="toolbar"><div class="search-wrap">${ic('search')}<input class="search-input" type="search" placeholder="Cari..." aria-label="Cari"></div>${chips}</div>`;
}
function applyListFilter() {
  ...
  if (shown === 0) { if (!none) { none = document.createElement('div'); none.className = 'empty list-none'; none.innerHTML = `<span class="empty-ic">${ic('search')}</span><p>Tidak ada hasil yang cocok.</p>`; list.appendChild(none); } }
  else if (none) { none.remove(); }
}
function wrapList(html, statuses) { return listToolbar(statuses) + `<div class="list">${html}</div>`; }
...
const Views = {
  async beranda(user) {
    ...
    return `
      <div class="hero-card"><div class="hero-greet">
        ...
        <span class="hero-role">👤 Warga</span></div></div>
      ...
      <div class="stat-grid">
        ...
        ${stat('Notifikasi saya', myCount, ic('bell'), 'green', 'notifikasi')}</div>
      ${pinned ? `<div class="section-title">📢 Pengumuman Terbaru</div><div class="card"><div class="card-body"><strong>${esc(pinned.judul)}</strong><p class="muted">${esc(pinned.isi)}</p><small class="muted">${tgl(pinned.createdAt)}</small></div></div>` : ''}
      ...
      <div class="quick-grid">
        ...
        <button class="quick" data-nav="kontak"><span class="q-ic">${ic('phone')}</span><span>Kontak Penting</span></button></div>`;
  },

  async _berandaPengurus(user) {
    const [lap, ban, sur, war] = await Promise.all([DB.list('laporan'), DB.list('bantuan'), DB.list('surat'), DB.list('warga')]);
    const jam = new Date().getHours();
    const sapa = jam < 11 ? 'Selamat pagi' : jam < 15 ? 'Selamat siang' : jam < 18 ? 'Selamat sore' : 'Selamat malam';
    ...
    return `
      <div class="hero-card"><div class="hero-greet">
        ...
        <span class="hero-role">🛡️ Pengurus</span></div></div>
  ...
  async menu(user) {
    ...
    const items = ids.map((id) => `<button class="quick" data-nav="${id}"><span class="q-ic">${ic(VIEWS[id].icon)}</span><span>${VIEWS[id].label}</span></button>`).join('');
    return `<div class="section-title">Semua Layanan</div><div class="quick-grid">${items}</div>
      <div class="card"><div class="card-body profile-row"><div class="avatar big">${esc((user.nama || 'U')[0].toUpperCase())}</div>
        ...
        <button class="btn ghost danger" id="btn-logout">Keluar</button></div></div>
  ...
  async pengumuman(user) {
    ...
    const form = role === 'pengurus' ? `
      <form id="f-peng" class="form card"><div class="card-body"><h3>Buat Pengumuman</h3>
        ...
        <button class="btn primary" type="submit">Terbitkan</button></div></form>` : '';
    const cards = rows.map((r) => `
      <div class="card" data-text="${esc((r.judul || '') + ' ' + (r.isi || '') + ' ' + (r.kategori || '') + ' ' + (r.author || ''))}"><div class="card-body">
        ...
        <p class="muted">${esc(r.isi)}</p><small class="muted">${esc(r.author || 'Pengurus')} • ${tgl(r.createdAt)}</small>
        ${role === 'pengurus' ? delBtn('pengumuman', r.id) : ''}</div></div>`).join('');
  ...
  async warga(user) {
    ...
    const head = `<div class="stat-grid">
      ...
      <div class="stat" data-tone="green"><div class="stat-ic">${ic('users')}</div><div><div class="stat-val">${totalAnggota}</div><div class="stat-lbl">Total Jiwa</div></div></div></div>`;
    const form = role === 'pengurus' ? `
      <form id="f-warga" class="form card"><div class="card-body"><h3>Tambah Data Warga</h3>
        ...
        <button class="btn primary" type="submit">Simpan</button></div></form>` : `<div class="callout blue"><div>ℹ️</div><div>Data warga dikelola oleh pengurus RT/RW.</div></div>`;
    const cards = rows.map((r) => `
      <div class="card" data-text="${esc((r.nama || '') + ' ' + (r.nik || '') + ' ' + (r.alamat || '') + ' RT' + (r.rt || '') + ' RW' + (r.rw || '') + ' ' + (r.status || '') + ' ' + (r.telp || ''))}"><div class="card-body">
        ...
        ${role === 'pengurus' ? delBtn('warga', r.id) : ''}</div></div>`).join('');
  ...
  async cariwarga(user) {
    ...
    const cards = rows.map((r) => `
      <div class="card" data-text="${esc((r.nama || '') + ' ' + (r.nik || '') + ' ' + (r.kk || '') + ' ' + (r.alamat || '') + ' RT' + (r.rt || '') + ' RW' + (r.rw || '') + ' ' + (r.status || '') + ' ' + (r.telp || ''))}"><div class="card-body">
        ...
        <div class="kv"><span>Anggota</span><b>${esc(r.jmlAnggota || '-')} jiwa</b></div>
        ${r.telp ? `<div class="chips"><a class="chip link" href="tel:${esc(r.telp)}">📞 ${esc(r.telp)}</a></div>` : ''}</div></div>`).join('');
  ...
  async surat(user) {
    ...
    const jenis = ['Surat Pengantar RT', 'Surat Pengantar RW', 'Cap / Legalisir RT', 'Surat Domisili', 'Surat Keterangan Tidak Mampu (SKTM)', 'Surat Pengantar KTP', 'Surat Pengantar KK', 'Surat Keterangan Usaha', 'Surat Izin Keramaian', 'Surat Pengantar Nikah (N1-N4)', 'Surat Pengantar SKCK', 'Surat Pindah Domisili', 'Surat Keterangan Kelahiran', 'Surat Keterangan Kematian', 'Surat Keterangan Belum Menikah', 'Lainnya'];
    ...
    const form = role !== 'pengurus' ? `
      <form id="f-surat" class="form card"><div class="card-body"><h3>Ajukan Surat / Layanan</h3>
        ...
        <button class="btn primary" type="submit">Kirim Pengajuan</button></div></form>` : '';
    const cards = rows.map((r) => {
      const cat = Array.isArray(r.catatan) ? r.catatan : (r.catatan ? [{ text: r.catatan }] : []);
      const last = cat.length ? cat[cat.length - 1] : null;
      return `
  ...
  async jadwal(user) {
    ...
    const form = role === 'pengurus' ? `
      <form id="f-jadwal" class="form card"><div class="card-body"><h3>Tambah Jadwal</h3>
        ...
        <button class="btn primary" type="submit">Simpan Jadwal</button></div></form>` : '';
    ...
    const list = rows.map((r) => `
      <div class="card"><div class="card-body">
        ...
        ${role === 'pengurus' ? delBtn('jadwal', r.id) : ''}</div></div>`).join('') || emptyState('Belum ada jadwal.', 'calendar');
  ...
  async kontak(user) {
    ...
    const form = role === 'pengurus' ? `
      <form id="f-kontak" class="form card"><div class="card-body"><h3>Tambah Kontak</h3>
        ...
        <button class="btn primary" type="submit">Simpan Kontak</button></div></form>` : '';
    ...
    const cards = rows.map((r) => `
      <div class="card" data-text="${esc((r.nama || '') + ' ' + (r.peran || '') + ' ' + (r.kategori || '') + ' ' + (r.telp || ''))}"><div class="card-body contact-row">
        ...
        <a class="btn ghost" href="tel:${esc(r.telp)}">📞 Telp</a></div>${role === 'pengurus' ? `<div class="card-body pt0">${delBtn('kontak', r.id)}</div>` : ''}</div>`).join('');
  ...
  async polling(user) {
    ...
    const form = role === 'pengurus' ? `
      <form id="f-polling" class="form card"><div class="card-body"><h3>Buat Polling</h3>
        ...
        <button class="btn primary" type="submit">Terbitkan Polling</button></div></form>` : '';
    const list = rows.map((r) => {
      const total = (r.opsi || []).reduce((s, o) => s + (o.votes || 0), 0);
      const voted = Voted.has(r.id) || r.status === 'Ditutup';
      const bars = (r.opsi || []).map((o, i) => {
        const pct = total ? Math.round((o.votes || 0) / total * 100) : 0;
        return `<button class="poll-opt" ${voted ? 'disabled' : ''} data-vote="${r.id}:${i}"><div class="poll-bar" style="--pct:${pct}%"></div><span class="poll-txt">${esc(o.text)}</span><span class="poll-pct">${voted ? pct + '%' : 'Pilih'}</span></button>`;
      }).join('');
      return `<div class="card"><div class="card-body"><div class="row-between"><strong>${esc(r.judul)}</strong>${badge(r.status)}</div>
        <div class="poll">${bars}</div><small class="muted">${total} suara • ${tgl(r.createdAt)}</small>
        ${role === 'pengurus' ? `<div class="actions">${r.status === 'Dibuka' ? `<button class="btn ghost mini" data-set="polling:${r.id}:Ditutup">Tutup</button>` : `<button class="btn ghost mini" data-set="polling:${r.id}:Dibuka">Buka lagi</button>`}<button class="btn ghost danger mini" data-del="polling:${r.id}">Hapus</button></div>` : ''}</div></div>`;
  ...
  async statistik(user) {
    ...
    const bar = (label, val, max, cls) => { const pct = max ? Math.round(val / max * 100) : 0; return `<div class="bar-row"><span class="bar-lbl">${esc(label)}</span><div class="bar-track"><div class="bar-fill ${cls || ''}" style="width:${pct}%"></div></div><span class="bar-val">${val}</span></div>`; };
    ...
    return `
      <div class="stat-grid">${sc(war.length, 'Kepala Keluarga', 'home', 'blue')}${sc(totJiwa, 'Total Jiwa', 'users', 'green')}${sc(aktif, 'Laporan Aktif', 'clipboard', 'orange')}${sc(suratPending, 'Surat Menunggu', 'mail', 'red')}</div>
      ...
      <div class="card"><div class="card-body"><h3>Ringkasan Lain</h3>
        ...
        <div class="kv"><span>Iuran belum bayar</span><b>${belum}</b></div></div></div>`;
  },
  ...
  async notifikasi(user) {
    ...
    const mineList = mine.map((m) => `<div class="card"><div class="card-body"><div class="row-between"><strong><span class="inline-ic">${ic(m.icon)}</span> ${esc(m.label)}</strong>${badge(m.status)}</div><small class="muted">${esc(m.t)} • ${tgl(m.at)}</small></div></div>`).join('') || emptyState('Belum ada pengajuan atas nama Anda.', 'bell');
    const info = peng.slice(0, 6).map((p) => `<div class="card"><div class="card-body"><div class="row-between"><strong><span class="inline-ic">${ic('megaphone')}</span> ${esc(p.judul)}</strong>${p.pinned ? '<span class="chip">📌</span>' : ''}</div><p class="muted">${esc(p.isi)}</p><small class="muted">${tgl(p.createdAt)}</small></div></div>`).join('') || emptyState('Belum ada pengumuman.', 'megaphone');
    const notifBtn = (Notif.supported() && Notification.permission !== 'granted') ? `<button class="btn primary block" data-notif-enable>🔔 Aktifkan Notifikasi</button>` : `<div class="callout green"><div>✅</div><div>Notifikasi aktif di perangkat ini.</div></div>`;
    return `<div class="card"><div class="card-body">${notifBtn}</div></div><div class="section-title">Status Pengajuan Saya</div>${mineList}<div class="section-title">Info & Pengumuman</div>${info}`;
  }
};
...
async function openItemModal(coll, id) {
  ...
  const sendCat = async () => {
    ...
    const arr = Array.isArray(s.catatan) ? s.catatan.slice() : [];
  ...
  wrap.addEventListener('click', async (ev) => {
    ...
    if (ev.target.closest('[data-catsend]')) return sendCat();
    const st = ev.target.closest('[data-sstatus]');
    if (st) {
    ...
    if (ev.target.closest('[data-cetak]')) {
      const nomEl = wrap.querySelector('#surat-nomor');
      const sNow = Object.assign({}, s, nomEl ? { nomor: (nomEl.value.trim() || s.nomor) } : {});
      const signer = user.role === 'pengurus' ? { nama: user.nama, jabatan: user.jabatan || '' } : { nama: s.approverNama || (CFG.PENGURUS && CFG.PENGURUS[0] && CFG.PENGURUS[0].nama) || 'Pengurus', jabatan: s.approverJabatan || (CFG.PENGURUS && CFG.PENGURUS[0] && CFG.PENGURUS[0].jabatan) || 'Ketua RT/RW' };
      cetakSurat(sNow, w, signer); return;
    }
  });
...
function cetakSurat(s, w, signer) {
  ...
  const seq = (String((s.id || '')).replace(/\D/g, '').slice(-3) || '001');
  const nomor = (s.nomor && String(s.nomor).trim()) ? String(s.nomor).trim() : (seq.padStart(3, '0') + '/RT-RW/' + ROMAN[now.getMonth() + 1] + '/' + now.getFullYear());
  const tglSurat = now.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
  const cat = Array.isArray(s.catatan) ? s.catatan : (s.catatan ? [{ text: s.catatan }] : []);
  const lastCat = cat.length ? cat[cat.length - 1].text : '';
  const sgNama = (signer && signer.nama) || 'Pengurus';
  const sgJab = (signer && signer.jabatan) || 'Ketua RT/RW';
  const wil = CFG.WILAYAH.nama || '';
  const kota = (wil.split('/').pop() || '').trim() || wil;
  const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><title>${esc(s.jenis)} - ${esc(nama)}</title><style>
    *{box-sizing:border-box;font-family:'Times New Roman',Georgia,serif;color:#000}
    ...
    .kop-sub{font-size:15px}
    .kop-addr{font-size:12px}
    .kop-line{border:none;border-top:3px double #000;margin:8px 0 20px}
    .meta td{font-size:14px;padding:1px 0}
    .judul{text-align:center;font-size:18px;font-weight:bold;text-decoration:underline;margin:20px 0 4px}
    .judul-sub{text-align:center;font-size:13px;margin-bottom:16px}
    ...
    <hr class="kop-line"/>
    <table class="meta"><tr><td>Nomor</td><td>: ${esc(nomor)}</td></tr><tr><td>Lampiran</td><td>: -</td></tr><tr><td>Perihal</td><td>: ${esc(s.jenis)}</td></tr></table>
    <div class="judul">SURAT KETERANGAN</div><div class="judul-sub">${esc(s.jenis)}</div>
    <p>Yang bertanda tangan di bawah ini, selaku ${esc(sgJab)} di lingkungan ${esc(wil)}, dengan ini menerangkan bahwa:</p>
    <table class="data"><tr><td class="l">Nama</td><td>:</td><td>${esc(nama)}</td></tr><tr><td class="l">NIK</td><td>:</td><td>${esc(nik)}</td></tr><tr><td class="l">Alamat</td><td>:</td><td>${esc(alamat)}</td></tr></table>
    <p>adalah benar warga yang berdomisili di lingkungan kami. Surat keterangan ini dibuat untuk keperluan:</p>
    <p class="kep">"${esc(s.keperluan || '-')}"</p>
    ${lastCat ? `<p><strong>Catatan dari pengurus:</strong> ${esc(lastCat)}</p>` : ''}
    <p>Demikian surat keterangan ini dibuat dengan sebenarnya, agar dapat dipergunakan sebagaimana mestinya.</p>
    <div class="ttd"><div>${esc(kota)}, ${esc(tglSurat)}</div><div>${esc(sgJab)}</div><div class="sp"></div><div><strong><u>${esc(sgNama)}</u></strong></div></div>
...
async function render(view) {
  ...
  $('#app').innerHTML = `
    <header class="topbar"><div><div class="tb-title">${ic(VIEWS[view].icon)} ${esc(headTitle)}</div><div class="tb-sub">${esc(CFG.WILAYAH.nama)}</div></div>
      <div class="topbar-actions"><button class="icon-btn" data-theme-toggle title="Ganti tema">${ic(Theme.get() === 'dark' ? 'sun' : 'moon')}</button>
        <button class="avatar" id="btn-profile" title="Menu & profil">${esc((user.nama || 'U')[0].toUpperCase())}</button></div></header>
  ...
  } catch (err) { console.error(err); $('#content').innerHTML = emptyState('Gagal memuat halaman. Coba lagi.', 'bell'); }
}
...
function renderLogin(mode) {
  ...
  $('#app').innerHTML = `
    <div class="login"><div class="login-card">
      <div class="login-logo">🏘️</div><h1>${esc(CFG.APP_NAME || 'Aplikasi')}</h1>
  ...
  if (reg) reg.addEventListener('submit', async (e) => {
    ...
    if (wargaList.some((wx) => String(wx.email || '').toLowerCase() === email)) { toast('Email sudah terdaftar, silakan login.'); return; }
    await DB.add('warga', { nama: d.nama, nik: d.nik, email: d.email, pass: d.pass, status: 'Tetap', jmlAnggota: 1 });
    toast('Pendaftaran berhasil ✅ Silakan login.'); renderLogin('login');
  });
  const log = $('#f-login');
  if (log) log.addEventListener('submit', async (e) => {
    ...
    if (peng) { Session.set({ nama: peng.nama || 'Pengurus', nik: peng.nik || '', email: peng.email || d.email, role: 'pengurus', jabatan: peng.jabatan || '' }); toast('Selamat datang, ' + (peng.nama || 'Pengurus')); render('beranda'); return; }
    const wargaList = await DB.list('warga');
    const warga = wargaList.find((wx) => String(wx.email || '').toLowerCase() === email && String(wx.pass || '') === d.pass);
    if (warga) { Session.set({ nama: warga.nama, nik: warga.nik || '', email: warga.email, role: 'warga' }); toast('Selamat datang, ' + warga.nama); render('beranda'); return; }
